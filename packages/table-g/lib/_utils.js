...
function renderColumnItem(item, text, record, index) {
  ...
  if (wrappedCilItem.showTip) return (
    ...
    }, _react.default.createElement("span", null, _text, " "))
  );
  return _text;
}
...
var useRowSelection = exports.useRowSelection = function useRowSelection(rowSelection, dataSource, bordered) {
  ...
  var onSelectRow = (0, _react.useCallback)(function (record, selected) {
    ...
    if (typeof rowSelection.onSelect === 'function') {
      var rows = isMultiple ? getFlatRecords([record]) : record;
      // 获取纯净数据
      var pureRows = getPureList(rows);
...
var usePagination = exports.usePagination = function usePagination(pagination, rowselection) {
...
var setStorageWidth = exports.setStorageWidth = function setStorageWidth(ele, tableKey, key) {
...
var getStorageWidth = exports.getStorageWidth = function getStorageWidth(key) {
  if (key) {
 ...
 */
var toggleFixedTable = exports.toggleFixedTable = function toggleFixedTable(tableParent) {
  // 找打content区域
...
var setStyle = exports.setStyle = function setStyle(dom, text) {
  if (!dom) return;
 ...
 * @param {*} hide 隐藏还是显示
 */
var toggleFixedTableScroll = exports.toggleFixedTableScroll = function toggleFixedTableScroll(fix) {
  ...
  if (fix) {
    ...
    var inner = fix.querySelector('.ant-table-body-inner');
...
var switchIndex = exports.switchIndex = function switchIndex(_ref2, from, to) {
...
var setMainTableBorder = exports.setMainTableBorder = function setMainTableBorder(table, headerFixed, dataLength) {
...
var originKey = exports.originKey = "__origin";
/**
 * 计算序号和可展开rowKey
 */
var computeIndexAndRowKey = exports.computeIndexAndRowKey = function computeIndexAndRowKey(list, computedRowKey) {
  var rowKey = [];
  var index = 0;
  var items = [{
    root: true,
...
var cloneDatasource = exports.cloneDatasource = function cloneDatasource(dataSource) {
 ...
 */
var diffList = exports.diffList = function diffList(oldList, newList) {
  ...
  newList.reduce(function (result, newItem) {
    ...
    if (isAdd) {
      ...
      if (isTree) {
        ...
        ni = _lodash.default.omit(newItem, 'children');
        // 比较子树
 ...
 * @param {*} cols 列数据
 */
var getComputedColIndex = exports.getComputedColIndex = function getComputedColIndex(cols) {
 ...
 * @param {number} parent
 */
var computeIndex = exports.computeIndex = function computeIndex(list) {
  ...
  var copyList = _lodash.default.cloneDeep(list);
  // 所有数据的key
  var renderRowKeys = [];
  // 所有数据平铺结构
  var tilingList = [];
  ...
  var _loop = function _loop() {
    ...
    if (!node['g-root']) {
      ...
      // 关联父级节点
      if (gprk !== undefined) {
        defineProperties(node, (0, _defineProperty2.default)((0, _defineProperty2.default)({}, "g-parent-row-key", gprk), "g-parent", gp));
      } else {
        // 根节点
 ...
 * @param length
 */
var getListRange = exports.getListRange = function getListRange(list, start, length) {
  // list.slice(start, start + length);
  var count = list.length;
  return list.slice(Math.min(start, count - length), start + length);
};
var getVirtualList = exports.getVirtualList = function getVirtualList(start, length, renderRowKeys, list) {
...
// 获取纯净数据
var getPureRecord = exports.getPureRecord = function getPureRecord(record) {
...
var getPureList = exports.getPureList = function getPureList(list) {
