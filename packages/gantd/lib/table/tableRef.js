    ...
    if (table && table.parentElement) {
      ...
      if (parent.classList.contains('ant-table-body')) {
        setTimeout(function () {
          ...
          // 纵向滚动
          var scrollY = parent.offsetWidth > parent.clientWidth; // 横向滚动

          var scrollX = parent.offsetHeight > parent.clientHeight; // fix bug:处理不同数据数量下的固定列滚动条的显示与否

          (0, _utils.toggleFixedTable)(parent, scrollY, scrollX, hasScrollY); // 根据是否出现纵向滚动条来设置header的overflow-y
          // 通过parent的宽度比较
          ...
          if (header) {
            ...
            (0, _utils.setStyle)(header, "overflow-y: ".concat(scrollY ? 'scroll' : 'hidden')); // fix bug:同时设置负margin，让head和body贴合
