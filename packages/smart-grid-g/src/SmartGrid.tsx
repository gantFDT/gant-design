...
function SmartGrid<T>(smartGridProps: SmartGridProps<T>): React.ReactElement {
  ...
  const [lastViewKey, setLastViewKey] = useLocalStorage<string>(
    `grid-last-view-key:${originGridKey}:${userId}`,
    '',
  );
  const [customViews, setCustomViews] = useLocalStorage<ViewConfig[]>(
    `grid-custom-views:${originGridKey}:${userId}`,
    [] as ViewConfig[],
  );
  const [companyViews, setCompanyViews] = useLocalStorage<ViewConfig[]>(
    `grid-company-views:${originGridKey}`,
    [] as ViewConfig[],
  );
  ...
  useEffect(() => {
    ...
    if (!originGridKey) {
      ...
      usedView =
        initView ||
        [...systemViews, ...companyViews, ...customViews].find(
          (sV: ViewConfig) => sV.viewId === viewKey,
        );
    }
  ...
  const handlerSaveViews = useCallback(params => {
    ...
    if (operateView.viewId.startsWith('company')) {
      ...
      setCustomViewsProp &&
        setCustomViewsProp(`grid-custom-views:${originGridKey}:${userId}`, views);
      setCustomViews(views);
    }
    hideModal && hideModal();
  }, []);
  ...
  useEffect(() => {
    setCustomViews((_customViews: ViewConfig[]) => {
      ...
      if (
        setCustomViewsProp &&
        originCustomViewsRef.current &&
        !JSONisEqual(originCustomViewsRef.current, _customViews)
      ) {
    ...
    setCompanyViews((_companyViews: ViewConfig[]) => {
      ...
      if (
        setCompanyViewsProp &&
        originCompanyViewsRef.current &&
        !JSONisEqual(originCompanyViewsRef.current, _companyViews)
      ) {
