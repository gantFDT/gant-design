...
const withSelector = compose(
  ...
  withHandlers({
    // 依赖转化后的value
    transformDataToList: ({ getLabel, getValue, renderItem, hideSelected, isMultiple, value: comValue }) => list =>
      list.map(item => {
        if (renderItem) {
        ...
        if (isPlainObject(item)) {
          ...
          return (
            <Option key={key} value={value} disabled={disabled} title={title} style={style} className={className}>
              {label}
            </Option>
          );
        }
  ...
  // 更新选项列表
  //#region
  withPropsOnChange(['dataList', 'filter', 'storageList', 'loading'], ({ dataList, filter, storageList, selectorStorageId, cleanStorage, transformDataToList, setStorageList, updateStorage, forceUpdateStorageList, loading, useStorage, query, labelProp, getLabel, isFilter }) => {
    let result = dataList;
    if (!query && filter && isFilter) {
      ...
      try {
        result = dataList.filter(item => {
          const LastIndex = filter.split('').reduce((index, char) => {
            if (index === -1) return -1;
            const label = getLabel(item);
            if (!label) {
...
class BasicSelector<T, R> extends PureComponent<SelectorInnerProps<T, R>> {
  ...
  renderSelect = () => {
    ...
    const select = (
      <Select showArrow loading={loading} {...props} onFocus={onFocus} ref={setSelectRef} className={classnames('gant-selector', className, !wrap && 'gant-selector-no-wrap')} onSearch={onSearch} onSelect={onSelect} onChange={onChange} onDropdownVisibleChange={onopen} labelInValue filterOption={false} dropdownClassName={classnames(dropdownClassName, 'gant-selector-dropdown')}>
        {children || renderList}
      </Select>
    );
    return select;
