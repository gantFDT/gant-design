...
export function useLocalStorage<T>(storageKey: string, initValue: T): [T, (params: T) => void] {
  ...
  const setLocalStorage = useCallback((list: T) => {
    ...
    localStorage.setItem(
      storageKey,
      JSON.stringify(Array.isArray(list) ? list : Object.assign({}, localData, list)),
    );
  }, []);
...
export const usePagination = (
...
): PaginationConfig | undefined | boolean => {
  const {
    pagination,
    pageIndex = 1,
    pageSize = 50,
  ...
  const handlerPageChange = useCallback(
    (page: number = 1, pageSize: number = 50): void => {
...
export const useTableConfig = (props: useTableConfigProps) => {
  ...
  // 行选中开关
  const { clickable, columnFields = [] } = tableConfig;
