...
function ColorPicker(props) {
  ...
  return !edit ? (
    <div
      className={`${prefixCls}-onlypreview`}
      style={{ backgroundColor: currentColor, width: width !== 'auto' ? width : 80 }}
    >
      #{showText}
    </div>
  ) : (
    <div className={`${prefixCls}-mainwrap`} style={{ width }}>
      ...
      >
        {disabled ? (
          <div className={`${prefixCls}-preview-text`}>#{showText}</div>
        ) : (
          <>
            <div
      ...
      {PrimaryColors.map(({ id, primary, children }) => {
        return (
          <div
