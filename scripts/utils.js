...
const utils = {
  *sleep(time) {
  ...
  resolveContent(content) {
    let res = content;
    res = res.replace("import Grid from '@grid'", "import {Grid} from 'gantd'");
    res = res.replace("import Header from '@header'", "import {Header} from 'gantd'");
    res = res.replace("'@grid'", "'gantd/lib/grid'");
    res = res.replace("'@gantd'", "'gantd'");
    res = res.replace("'@data-cell'", "'gantd'");
    res = res.replace(/\`/g, "\\`");
    res = res.replace(/\${(.*)}/g, '\\${$1}');
