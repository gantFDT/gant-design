export default {
  IEVersion: `export function IEVersion() {
  ...
  // 获取cookie、
  getCookie: `export function getCookie(name: string): string | null {
  ...
  // 删除cookie
  delCookie: `export function delCookie(name: string): void {
    var exp = new Date();
  ...
  setCookie: `export function setCookie(name: string, value: string, time: any = '', path: string = ''): void {
    if (time && path) {
  ...
  */
  ...
  export function getIdsFormTree(Data: any[], childrenKey: string = 'children', field: string = 'id'): any[] {
    let V; let L; let IDs = [];
    for (L = Data.length; L;) {
      V = Data[--L];
      IDs.push(V[field])
