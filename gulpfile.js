  ...
  dirs.forEach((dirName, idx) => {
    if (dirName) {
      if (method === 'import') {
        ...
        }${transToCamelCase(dirName)}'`;
      } else {
        str2replace += `${idx ? '\n' : ''}${method} { default as ${dirName} } from '${
          repeat ? '../'.repeat(repeat) : './'
        }${transToCamelCase(dirName)}'`;
      }
    }
    ...
    function compileScriptToGantd() {
      return (
        src([
          ...
          .pipe(tsProject())
          .pipe(
            dest(`packages/gantd/${targetDir}/${__dirName === 'data-cell' ? '' : __dirName + '/'}`),
          )
          .pipe(filter(file => !file.path.endsWith('.d.ts')))
          .pipe(src([`packages/${dirName}/src/**/*.jsx`, `packages/${dirName}/src/**/*.js`]))
          .pipe(
...
task('code', function code() {
  ...
  return src([`stories/*/index.js`, `stories/*/index.jsx`])
    .pipe(
      through2.obj(function(file, enc, next) {
        let content = file.contents.toString();
...
//监听的demo
const watchFiles = [`stories/*/demo/*/index.tsx`];
//生成demo的code
task('gencode', function code() {
  return src(watchFiles)
    .pipe(
      through2.obj(function(file, enc, next) {
        let content = file.contents.toString();
        content = Utils.resolveContent(content);
        let fileContent = `export default \` \n${content} \n \``;
        const buf = Buffer.from(fileContent);
        file.contents = buf;
        this.push(file);
...
task('watch:code', function() {
  watch(watchFiles, series('gencode'));
});
