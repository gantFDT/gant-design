...
function getDirNames (__path) {
  return fs
    .readdirSync (__path, {withFileTypes: true})
    .filter (
      item =>
        item.isDirectory () && item.name !== 'style' && item.name !== 'compose'
    )
    .map (item => item.name);
}
...
function ScriptTask (dirName) {
  return series (
    function compileScript () {
      return (
        src ([
          `packages/${dirName}/src/**/*.tsx`,
          `packages/${dirName}/src/**/*.ts`,
        ])
          .pipe (tsProject ())
          ...
          .pipe (
            src ([
              `packages/${dirName}/src/**/*.jsx`,
              `packages/${dirName}/src/**/*.js`,
            ])
          )
          .pipe (
            // 处理DataCell路径问题
          ...
          .pipe (
            ...
            through2.obj (function (file, enc, next) {
              ...
              if (~content.indexOf (`@`)) {
                ...
                if (dirName === 'gantd') {
                  const Idx = filePath.lastIndexOf (dirName);
                  const filelevel = filePath.slice (Idx).split (pathSplit)
                    .length;
                  const pathPrefix = filelevel > 3
                    ? '../'.repeat (filelevel - 3)
                    : './';
    ...
    function copyJson () {
      return src (`packages/${dirName}/src/**/*.json`).pipe (
        dest (`packages/${dirName}/lib/`)
      );
    },
  ...
  return series (
    function cleanGantd (cb) {
      if (__dirName === 'data-cell') {
        const dataCellDirs = getDirNames (
          path.resolve (__dirname, `packages/data-cell-g/src/`)
        );
        dataCellDirs.forEach (___dirName => {
          rimraf.sync (
            path.resolve (
              __dirname,
              `packages/gantd/${targetDir}/${___dirName}/`
            )
          );
        });
      } else if (__dirName === 'color-picker') {
        rimraf.sync (
          path.resolve (__dirname, `packages/gantd/${targetDir}/_color-picker/`)
        );
      } else {
        rimraf.sync (
          path.resolve (__dirname, `packages/gantd/${targetDir}/${__dirName}/`)
        );
      }
      cb ();
    },
    function compileScriptToGantd () {
      return (
        src ([
          `packages/${dirName}/src/**/*.tsx`,
          `packages/${dirName}/src/**/*.ts`,
          `!packages/data-cell-g/src/index.tsx`,
        ])
          .pipe (tsProject ())
          .pipe (
            dest (
              `packages/gantd/${targetDir}/${__dirName === 'data-cell' ? '' : __dirName + '/'}`
            )
          )
          .pipe (filter (file => !file.path.endsWith ('.d.ts')))
          .pipe (
          ...
          .pipe (
            ...
            through2.obj (function (file, enc, next) {
              ...
              const filelevel = filePath.slice (Idx).split (pathSplit).length;
              content = resolveDataCellPath (
                content,
                dirName !== 'data-cell-g' ? filelevel - 1 : 1
              );
              const buf = Buffer.from (content);
              file.contents = buf;
              this.push (file);
          ...
          .pipe (
            ...
            through2.obj (function (file, enc, next) {
              let content = file.contents.toString ();
              const filePath = file.path;
              const isStyleFile = filePath.endsWith (
                `style${pathSplit}index.js`
              );
              ...
              if (~content.indexOf ('@')) {
                const Idx = filePath.indexOf (__dirName);
                const filelevel = filePath.slice (Idx).split (pathSplit).length;
                const pathPrefix = __dirName !== 'data-cell'
                  ? '../'.repeat (filelevel - 1)
                  : '../';
                if (isStyleFile) {
                  // 处理 */style/index.js 里面的 '@data-cell/*/style'
                  content = content.replace (
                    new RegExp ('@data-cell/', 'g'),
                    pathPrefix
                  );
                }
    ...
    function compileDTS () {
      return src (
        `packages/gantd/${targetDir}/${__dirName === 'data-cell' ? '' : __dirName + '/'}**/*.d.ts`
      )
        .pipe (
          // 处理DataCell路径问题
          through2.obj (function (file, enc, next) {
    ...
    function renameColorPicker (cb) {
      if (__dirName === 'color-picker') {
        fs.rename (
          'packages/gantd/lib/color-picker/',
          'packages/gantd/lib/_color-picker/',
          cb
        );
      } else {
        cb ();
...
packageNames.forEach (packageName => {
  ...
  task (
    ...
    series (
      ...
      ScriptTask (dirName),
      dirName !== 'gantd'
        ? CopileToGantdTask (dirName, 'lib')
        : function skipGant (cb) {
            cb ();
          }
    )
  );
...
task (
  'lib',
  series (...packageNames.map (packageName => `lib:${packageName.slice (9)}`))
);
    ...
    .pipe (
      through2.obj (function (file, enc, next) {
        ...
        if (~startIdx) {
          ...
          const Header = CodeHead.replace (/@/g, '').replace (
            /([`\$])/g,
            '\\$1'
          );
